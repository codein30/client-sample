...
class Bot {
    ...
    if (!this.config) {
      try {
        const gitToken = await kms.decrypt({ CiphertextBlob: Buffer.from(process.env.GIT_SECRET, 'base64') })
          .promise()
          .then(secret => {
            const token = Buffer.from(`saturn-codtm:${secret.Plaintext.toString()}`)
  ...
  async getsaturnDB (client) {
    if (!this.clients[client] || !this.clients[client].saturnDB) {
      const config = await this.getConfig(client).then(fig => fig.saturn.mysql)
      config.bus = this.bus
      this.clients[client].saturnDB = await mysql(config)
    }

    return this.clients[client].saturnDB
  }

  closesaturnDB (client) {
    if (this.clients[client].saturnDB) {
      return this.clients[client].saturnDB.end().then(() => {
        delete this.clients[client].saturnDB
      })
    }
  ...
  transform (botId, source, destination, each, config = {}) {
    ...
    return new Promise((resolve, reject) => {
      ...
      return this.closeAllsaturnDBs()
    }).then(() => {
      this.mappers = {}
    }).catch(async err => {
      await this.closeAllsaturnDBs()
      throw err
    })
  }
