...
class Contacts(Resource):
    ...
    def get(self):
        ...
        saturn-co_client = a.get_auth_saturn-co_client(domain)
        ...
        contacts = contacts_service.get_contacts(saturn-co_client=saturn-co_client, contact_ids=parse_csv(contact_ids))
        if contacts is None:
            return ResponseHelper.abort(410, msg=f'The recommendations for client {saturn-co_client} no longer exists')
        return contacts
    ...
    def post(self):
        ...
        try:
            for c in request_body['contacts']:
                ...
                Contact(**c)
                # Enrich payload
                c.update({'saturn-co_client': saturn-co_client, 'ingestion_timestamp': str(datetime.utcnow())})
