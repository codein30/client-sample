...
def test_move_files_by_moving_a_single_file_within_multiple_files(storage):
    ...
    blob1 = Blob('saturn-messages-final-doc1.txt', bucket=from_bucket)
    blob2 = Blob('saturn-contact-phone-numbers-final-doc2.txt', bucket=from_bucket)
    prefix = 'saturn-messages-final'
    blobs = [blob1, blob2]
...
def test_move_files_by_moving_multiple_files(storage):
    ...
    to_bucket = mock.create_autospec(Bucket)
    to_bucket.name = 'TO_BUCKET'
    blob1 = Blob('saturn-contact-phone-numbers-doc1.txt', bucket=from_bucket)
    blob2 = Blob('saturn-contact-phone-numbers-final-doc2.txt', bucket=from_bucket)
    prefix = 'saturn-contact-phone-numbers'
    blobs = [blob1, blob2]
...
def test_remove_prefix(storage):
    prefixed_string = 'gs://us-west3-saturn-co-ml-prd-5b4c8c18-bucket/dags'
    assert CloudStorage.remove_prefix(prefixed_string, 'gs://') == 'us-west3-saturn-co-ml-prd-5b4c8c18-bucket/dags'
